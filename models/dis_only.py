...
class Model(nn.Module):
    def __init__(self, pret=True, args=None):
        ...
        if option == 'resnet18':
            ...
            self.dim = 512
        if option == 'resnet50' or option == 'dis_only':
...
class Loss(nn.Module):
    ...
    def forward(self, logits, label, epoch):
        ...
        # st_loss1 = self.cls_loss(output_sts, label) st_loss2 = self.cls_loss(output_sts, label + self.nc)
        # st_cat_loss = 0.5 * st_loss1 + 0.5 * st_loss2 em_loss = self.em_loss(output_stt) st_dom_loss = 0.5 *
        # self.st_loss(output_stt, area='left') + 0.5 * self.st_loss(output_stt, area='right') + em_loss

        all_loss = loss1 + loss2 + 0.01 * entropy_loss
        # f_loss = loss1 + loss2 - self.eta * dis_loss + 0.01 * entropy_loss
        f_loss = loss1 + loss2 - self.eta * dis_loss + 0.01 * entropy_loss + domain_loss
        # g_loss = dis_loss
        # st_loss = st_cat_loss + lam * st_dom_loss
        g_loss = dis_loss
        return all_loss, f_loss, g_loss


def dis_only(pretrained=True, args=None):
